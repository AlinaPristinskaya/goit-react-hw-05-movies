...
const MovieDetailsPage=()=> {
    ...
    return <> 
             ...
             <div className={s.blokFilm}>
                  {idFilm&& <><img className={s.img} src={`https://image.tmdb.org/t/p/w300${idFilm.poster_path}`}
                   alt={idFilm.title}/>
                   <div className={s.details}>
                  <h3>{idFilm.title}</h3>
                  <p>User Score:{idFilm.vote_average}</p> 
                  <h4>Overview</h4><p>{idFilm.overview}</p>
                  {genres&& <><h4>Genres</h4><ul>{genres.map(item=><li key={item.id}>{item.name}</li>)}</ul></>}</div>            
                 </>} 
            </div>
            <div>
                 <NavLink
